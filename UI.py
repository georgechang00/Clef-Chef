...
class AudioSlider(BoxLayout):
    ...
    def on_resize(self, win_size):
        self.size = (.075*win_size[0], 250/1200.*win_size[1])
        self.pos = (self.pos[0]/1600*win_size[0], self.pos[1]/1200*win_size[1])
        self.label.height = -1.6*self.size[1]
...
class MainWidget1(BaseWidget):
    def __init__(self, back):
        ...
        self.slider_layout = BoxLayout(
            ...
            size_hint=(None, None),
            size=(.40625*Window.width, 280/1200.*Window.height),  # Increased size to accommodate larger sliders
            pos=(Window.width/2 - .25*Window.width, Window.height/32),  # Adjusted position to center
        ...
        self.reverb_slider = AudioSlider(
            ...
            max_value=5,
            value=0.0,
            pos = (.3125*Window.width,.0833*Window.height)
        )
        self.delay_slider = AudioSlider(
            ...
            max_value=1,
            value=0.1,
            pos = (.4375*Window.width,.0833*Window.height)
        )
        self.high_shelf_slider = AudioSlider(
            ...
            max_value=24,
            value=0,
            pos = (.5625*Window.width,.0833*Window.height)
        )
        ...
        self.low_shelf_slider = AudioSlider(
            ...
            max_value=24,
            value=0,
            pos = (.8125*Window.width,.0833*Window.height)
        )
    ...
    def on_touch_down(self, touch):
        ...
        for idx,inst in self.instrument_list.items():
            ...
            if dist<inst.ellipse.csize[0]/2.:
                inst.grabbed = True
    ...
    def on_resize(self, win_size):
        ...
        self.reverb_slider.on_resize(win_size)
        self.delay_slider.on_resize(win_size)
        self.high_shelf_slider.on_resize(win_size)
        self.mid_freq_slider.on_resize(win_size)
        self.low_shelf_slider.on_resize(win_size)
        self.slider_layout.size = (.40625*win_size[0], 280/1200.*win_size[1])
        self.slider_layout.pos = (win_size[0]/2 - .25*win_size[0], win_size[1]/32)
        self.slider_layout.padding = 50/1600.*win_size[0]
        self.main.on_resize(win_size)
        self.trash_area.cpos = (win_size[0]-100/1600.*win_size[0], 100/1200.*win_size[1])
        self.trash_area.csize = (100/1600.*win_size[0], 100/1200.*win_size[1])
        for idx, inst in self.instrument_list.items():
            inst.on_resize(win_size)
...
class PotObject(InstructionGroup): #Circle which houses instruments
    def __init__(self):
        ...
        self.pot = CRectangle(cpos = (Window.width/2,Window.height*5/8-10/1200.*Window.height), csize = (Window.width*.8, Window.height*.9), texture = Image('pot.png').texture)
        self.add(self.pot)
    ...
    def on_resize(self, win_size):
        ...
        self.x1 = Window.width/2-self.rad*.6
        self.x2 = Window.width/2+self.rad*.6
        self.pot.cpos = (Window.width/2,Window.height*5/8-10)
        self.pot.csize = (Window.width*.8, Window.height*.9)
        self.ellipse.cpos = (Window.width/2, Window.height*19/32+5)
        self.ellipse.csize = (self.rad*1.3, self.rad/4+11)
...
class InstrumentObject(InstructionGroup): # Should be updated with images of instruments
    def __init__(self, imagepath, cpos, audiopath, idx, pot, trash, add, rem, remove_sound):
        ...
        self.ogpos = cpos
        self.radius = 100/1600.*Window.width
        self.ellipse = CEllipse(cpos = cpos, csize = (self.radius,self.radius), texture=Image(imagepath).texture)
        self.grav_const = 0
        self.grabbed = False
    ...
    def on_resize(self, win_size):
        self.ogpos = (self.ogpos[0]/1600.*win_size[0],self.ogpos[1]/1200.*win_size[1])
        self.radius = 100/1600*win_size[0]
        self.ellipse.csize = (self.radius, self.radius)
