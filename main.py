...
def spectral_blend3(audio_list, sr, alpha1=0.5, alpha2=0.5):
    # Get all non-zero audio signals
    active_signals = []
    active_specs = []
    
    # First pass: compute STFTs for all non-zero signals
    for audio in audio_list:
        if not np.array_equal(audio, np.zeros_like(audio_list[0])):
            active_signals.append(audio)
            _, _, Zxx = stft(audio, fs=sr, nperseg=1024)
            active_specs.append(Zxx)
    
    if not active_specs:  # No active signals
        return np.zeros_like(audio_list[0])
    
    if len(active_specs) == 1:  # Only one active signal
        return active_signals[0]
    
    # Find minimum time bins across all spectrograms
    min_time_bins = min(spec.shape[1] for spec in active_specs)
    
    # Truncate all spectrograms to same length
    active_specs = [spec[:, :min_time_bins] for spec in active_specs]
    
    # Progressive blending
    result_Zxx = active_specs[0]
    for i in range(1, len(active_specs)):
        # Get magnitudes
        mag1 = np.abs(result_Zxx)
        mag2 = np.abs(active_specs[i])
        
        # Calculate phase weights
        phase_weight = mag1 / (mag1 + mag2 + 1e-10)
        
        # Blend magnitudes and phases
        blended_magnitude = (1 - alpha1) * mag1 + alpha1 * mag2
        blended_phase = phase_weight * np.angle(result_Zxx) + (1 - phase_weight) * np.angle(active_specs[i])
        
        # Update result
        result_Zxx = blended_magnitude * np.exp(1j * blended_phase)
    
    # Convert back to time domain
    _, blended_audio = istft(result_Zxx, fs=sr)
...
class SpectralSynth:
    ...
    def toggle_audio(self, audio_idx):
        ...
        print(f"Toggled audio {audio_idx}: {getattr(self, f'is_audio{audio_idx}')}")
        self.update_blend()
        self.stop_all_notes()
...
class BubbleButton(Button):
    ...
    def on_resize(self, win_size):
        self.pos = (win_size[0] * 0.85, win_size[1] * 0.88)
        self.size = (170/1600.*win_size[0], 90/1200.*win_size[1])
...
class CanvasWidget(Widget):
    def __init__(self, mainwidget, **kwargs):
        ...
        self.sounds_button = BubbleButton(
            text="Sounds",
            size_hint=(None, None),
        ...
        self.sounds_button.bind(on_press=self.open_sound_popup)
        self.add_widget(self.sounds_button)
    def on_resize(self, win_size):
        self.sounds_button.on_resize(win_size)        
        self.sound_x_position = 150/1600.*win_size[0]  # Fixed x position for all sounds
        self.slot_height = win_size[1] / 12.  # Divide the screen into 5 equal slots
    ...
    def add_sound_to_canvas(self, sound):
        ...
        inst = UI.InstrumentObject(sound['image'],(self.sound_x_position, y_position),sound['file'], idxslot, self.main_widget.pot,(Window.width-100/1600.*Window.width, 100/1200.*Window.height), self.main_widget.add_audio, self.main_widget.rem_audio,self.remove_sound_from_canvas)
...
class AudioMixerApp(App):
    def build(self):
        ...
        self.main_widget = UI.MainWidget1(self)
        ...
        self.canvas_widget = CanvasWidget(self.main_widget)
        ...
        return layout   
    def on_resize(self, win_size):
        self.canvas_widget.on_resize(win_size)
